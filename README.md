...
このワークフローを開始する前に、YubiKeyの初期設定と依存関係のインストールを行う。

1.  **依存関係のインストール (Fedora)**
    このワークフローは`gnupg`や`yubikey-manager`などのツールに依存している。以下のコマンドで必要なパッケージをインストールする。

    ```bash
    make install-deps
    ```

2.  **YubiKeyのPIN/リトライ回数設定**
...
2.  **YubiKeyの鍵属性を設定**
    このワークフローは `ed25519` 鍵を基本とする。以下のコマンドを実行し、YubiKeyが `ed25519` 鍵を受け入れられるように設定する。
...
4.  **スクリプトに実行権限を付与**
    `Makefile`経由で実行する場合、この手順は不要になったが、個別にスクリプトを実行したい場合は以下のコマンドが利用できる。
...
GPGの核となる「マスター鍵」を生成し、安全なUSBに保管する。`make`コマンドが`.env`を自動で読み込む。

```bash
make generate-primary-key USB=<マスターUSBのパス>
```
...
make import-keys USB=<マスターUSBのパス>
```

---
##### **操作: 副鍵の生成とYubiKeyへの移動**
```bash
# 1. 副鍵を生成し、バックアップUSBに保存
make add-subkeys USB=<マスターUSBのパス>
...
make generate-ssh-key USB=<マスターUSBのパス>
```
...
```bash
make backup-primary-key USB=<新しいバックアップUSBのパス>
```
...
```bash
make sync-backup SRC_USB=<マスターUSBのパス> DST_USB=<複製USBのパス>
```

##### **目的D: GPGキーの有効期限を延長する**
GPGキーの有効期限を1年延長する。この操作は主鍵のインポート（STEP 1）が必要。

```bash
make extend-key-limit USB=<マスターUSBのパス>
```

##### **目的E: ホストに公開鍵をインポートする**
新しいPCをセットアップする際など、マスターUSBからホストPCのGPG環境（`.gnupg`）へ公開鍵を直接インポートする。

```bash
make import-keys-to-host USB=<マスターUSBのパス>
```
...
Bash
# source .env
make restore-from-qr USB=<新しいUSBのパス> QR_DATA="読み取ったBase64文字列"
